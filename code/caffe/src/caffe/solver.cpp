...
void Solver<Dtype>::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
	...
	//LOG(INFO)<<"iter_iter_iter_iter_"<<iter_;
	...
	if ((iter_+1)%2==1){
		...
		for (int l = 47; l < net_->layers().size(); ++l) {
			Layer<Dtype>& layer = *net_->layers()[l].get();
			for (int b = 0; b < layer.blobs().size(); ++b) {
				Blob<Dtype>& blob = *layer.blobs()[b].get();
				caffe_set(blob.count(), static_cast<Dtype>(0), blob.mutable_cpu_diff());
     ...
     for (int l = 0; l < net_->layers().size(); ++l) {
        ...
        for (int b = 0; b < layer.blobs().size(); ++b) {
          ...
          for (int i = 0; i < blob.count(); ++i) {
            data_sum += blob_cpu_data[i];
            //(blob_cpu_data[i] > Dtype(0.)) ?     : - blob_cpu_data[i];
            diff_sum +=  blob_cpu_diff[i];
             //(blob_cpu_diff[i] > Dtype(0.)) ?   : - blob_cpu_diff[i];
          }
          data_sum /= blob.count();
          diff_sum /= blob.count();
